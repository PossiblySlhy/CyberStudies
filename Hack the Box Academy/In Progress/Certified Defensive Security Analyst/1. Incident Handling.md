...
- Divide actions into `short-term containment` and `long-term containment`
  ...
  - `Long-term containment` &rarr; focus on persistent actions and changes </br></br> actions include:
    ...
    - inserting host instrusion detection systems
...
Once the incident is contained, eradication is necessary to elimnate both the root cause of the incident and what is left of it to ensure that the adversary is out of the systems and network.
...
In the recovery stage, we bring systems back to normal operaton
